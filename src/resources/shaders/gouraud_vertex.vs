...
vec4 calcLightColour(vec3 light_colour, float light_intensity, vec3 position, vec3 to_light_dir, vec3 normal)
{
    ...
    float specularFactor = max( dot(camera_direction, reflected_light), 0.0); //Is this the interpolation?
...
void main()
{
    ...
    totalLight += calcDirectionalLight(directionalLight, position, mvNorm);
    totalLight += calcPointLight(pointLight, position, mvNorm); 
    
    VertexColor = baseColour * totalLight;
}
