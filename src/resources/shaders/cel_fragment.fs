...
vec4 calcLightColour(vec3 light_colour, float light_intensity, vec3 position, vec3 to_light_dir, vec3 normal)
{
    ...
    // Diffuse Light
    diffuseFactor = max(dot(normal, to_light_dir), 0.0);
    ...
    specularFactor = max( dot(camera_direction, reflected_light), 0.0);
...
void main()
{
    ...
    vec4 baseColour; 
    //if (material.useColour == 0) {
        baseColour = texture(texture_sampler, mvTextCood);
    //}
    //else {
    //    baseColour = vec4(material.colour, 1);
    //}

    float intensity = (0.6 * diffuseFactor) + (0.4 * specularFactor);
    ...
    fragColor = baseColour * intensity;
}