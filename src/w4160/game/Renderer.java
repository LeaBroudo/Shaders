...
import w4160.engine.graphics.Texture;
...
public class Renderer {
    ...
    private String texString = "src/resources/models/spot/spot_texture.png"; 
    ...
    public ShaderProgram createCheckerShader() throws Exception {
        ...
        shaderProgram.createVertexShader(new String(Files.readAllBytes(Paths.get("src/resources/shaders/checker_vertex.vs"))));
        shaderProgram.createFragmentShader(new String(Files.readAllBytes(Paths.get("src/resources/shaders/checker_fragment.fs"))));
        shaderProgram.link();
    ...
    public ShaderProgram createTextModShader() throws Exception {
        ShaderProgram shaderProgram = new ShaderProgram();

        shaderProgram.createVertexShader(new String(Files.readAllBytes(Paths.get("src/resources/shaders/textmod_vertex.vs"))));
        shaderProgram.createFragmentShader(new String(Files.readAllBytes(Paths.get("src/resources/shaders/textmod_fragment.fs"))));
        shaderProgram.link();
    ...
    public ShaderProgram createRainbowShader() throws Exception {
        ShaderProgram shaderProgram = new ShaderProgram();

        shaderProgram.createVertexShader(new String(Files.readAllBytes(Paths.get("src/resources/shaders/rainbow_vertex.vs"))));
        shaderProgram.createFragmentShader(new String(Files.readAllBytes(Paths.get("src/resources/shaders/rainbow_fragment.fs"))));
        shaderProgram.link();
    ...
    public void init(Window window) throws Exception {
        ...
        shaderProgramList.put("textmod", createTextModShader());
        shaderProgramList.put("normal", createNormalShader());
        shaderProgramList.put("cel", createCelShader());
        shaderProgramList.put("rainbow", createRainbowShader());
    ...
    public void render(Window window, Camera camera, GameItem[] gameItems, Vector3f ambientLight,
        PointLight pointLight, DirectionalLight directionalLight, String currentShader) throws Exception {
        ...
        // Student code
        ...
        else if(currentShader.equals("checker")) {
        	shaderProgram.setUniform("projectionMatrix", projectionMatrix);
        ...
        else if(currentShader.equals("textmod")) {
        	shaderProgram.setUniform("projectionMatrix", projectionMatrix);
            ...
            //Texture text = new Texture(texString);
            createTexture(gameItems);
        ...
        else if(currentShader.equals("rainbow")) {
        	shaderProgram.setUniform("projectionMatrix", projectionMatrix);
            ...
            int gifLen = 7; 
            int currPng = (int)(java.lang.System.currentTimeMillis() % (gifLen*50)); 
            Texture text = new Texture("src/resources/textures/rainbow_textures/rain" + Integer.toString(currPng/50) + ".png");

            shaderProgram.createUniform("angle");
            float angle = (float)(java.lang.System.currentTimeMillis() % (360*100)); 
    ...
    public void createTexture(GameItem[] gameItems) throws Exception {
        for(GameItem gameItem : gameItems) {
            
            Mesh mesh = gameItem.getMesh();
            try {
                Texture text = new Texture(mesh.getMaterial().getTexture().getFileName());
            }
            catch(Exception e) {}
        }
    }
