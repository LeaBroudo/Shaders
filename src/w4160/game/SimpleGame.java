...
public class SimpleGame implements IControlLogic {
    ...
    public SimpleGame(String meshFile) {
        ...
        lightAngle = -90;
        currentObj=0;
        this.meshFile = new String[]{ meshFile };
    }
     ...
     * 
     * @param meshFile .obj filename that stores the mesh
     * @param textfile .png filename that stores the texture
     * @see Mesh
     * @see OBJLoader
     */
    public SimpleGame(String meshFile, String textFile) {
    	renderer = new Renderer();
        camera = new Camera();
        camera.setPosition(0.f, 0.f, 0.6f);
        cameraInc = new Vector3f(0.0f, 0.0f, 0.0f);
        ...
        lightAngle = -90;
        currentObj=0;
        this.meshFile = new String[]{ meshFile, textFile };
    }
    ...
    public void init(Window window) throws Exception {
        ...
        Mesh mesh = meshFile==null ? new Mesh() : OBJLoader.loadMesh(meshFile[0]);
    
        //Material material = new Material(new Vector3f(0.4f, 0.8f, 1f), reflectance);
        //Material material = new Material("src/resources/models/spot/spot_texture.png", reflectance);
        Material material; 

        if (this.meshFile.length == 2) { 
            material = new Material(this.meshFile[1], reflectance);
        } 
        else {
